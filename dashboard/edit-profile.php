...
$cid=$_GET['cid'];
$cid = decodificar($cid);
$resultado = $con->query("SELECT cid,name_parson,name,email,phone,office,role,date,imagen,tipo_imagen FROM manager_admin WHERE cid='$cid' ");
																...
																if(isset($_SESSION['user_name'])) { // comprobamos que la sesión esté iniciada 
																	if(isset($_POST['enviar'])) {
																		if($_POST['pwd'] != $_POST['usuario_clave_conf']) {																		
																			...
																			$sql = mysql_query("UPDATE manager_admin SET pwd='".$pwd."' WHERE name='".$name."'"); 
