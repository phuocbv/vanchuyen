...
// *************************************************************************
	...
	$sql = "INSERT INTO courier_track (cid, tracking, letra, pick_time, status, comments, bk_time, ship_name, phone, correo, user)
			VALUES ($cid, '$cons_no', '$pre', '$pick_time | $citaddress', '$status', '$comments', '$deliverydate', '$ship_name', '$s_phone', '$email', '$deliveruser')";
	dbQuery($sql);
	
    $sql_1 = "UPDATE courier_online SET  deliverydate='$deliverydate' , status='$status', deliveruser='$deliveruser' WHERE cid = '$cid' AND cons_no = '$cons_no'";
	dbQuery($sql_1);
	
	$sql_2 = "UPDATE courier_customer SET  schedule='$deliverydate' , status='$status' WHERE  tracking = '$cons_no'";
	dbQuery($sql_2);
	...
	$settingssms =  mysql_query("SELECT detailsend,detailinvoice,detailprice FROM api_sms WHERE id='5'");
	...
	$info = $nexmo_sms->sendText($_POST['s_phone'], $_POST['ship_name'], ''.$ship_name.', '.$apia.' '.$status.'. '.$apib.' '.$deliverydate.'. '.$apic.' '.$comments.'');
