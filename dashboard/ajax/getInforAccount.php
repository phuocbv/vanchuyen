...
if(isset($_POST['type_account']) && !empty($_POST["country_id"])){
    //Get all state data
    ...
    if($rowCount > 0){
        ...
        echo '<option value="">Capital not available</option>';
    }
}
...
if(isset($_POST["customer_id"]) && !empty($_POST["customer_id"])) {
    ...
    if($rowCount > 0) {
        while($customer = $query->fetch_assoc()) {
            ...
            if(!isset($lockerid) && empty($lockerid)){
                $lockerid="";
            }
